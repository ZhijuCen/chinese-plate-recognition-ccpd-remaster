...
import sys
try:
    from ..utils.filename_parser import load_char_annots
except ValueError:
    sys.path.insert(0, "..")
    from utils.filename_parser import load_char_annots
from data.dataset import OCRDataset, concat_ds
from model.model_container import MobileOCRNetContainer
...
def _add_misc_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--char-annot-path", type=Path,
        help="Location of char annot file."
    )
    parser.add_argument(
        "--device", default="cuda",
        help="Device for training nn."
    )
    parser.add_argument(
...
def get_list_of_ds(dataset_locations: List[Path],
                   ...
                   batch_size: int = 16,
                   is_val = False):
    list_of_ds: List[OCRDataset] = list()
    for lf, ds_idx in zip(label_files, mappings):
        list_of_ds.append(OCRDataset(
            dataset_locations[ds_idx], lf, is_val=is_val))
    ds = concat_ds(*list_of_ds)
    return ds
...
def main(args: Namespace):
    ...
    if len(args.val_mapping) > 0:
        ds_val = get_list_of_ds(args.datasets,
                                ...
                                batch_size=args.batch_size,
                                is_val=True)
    char_annot = load_char_annots(args.char_annot_path)["char_annotations"]
    container = MobileOCRNetContainer(len(char_annot),
                                      runtime_output_dir=args.checkpoint_dir)
